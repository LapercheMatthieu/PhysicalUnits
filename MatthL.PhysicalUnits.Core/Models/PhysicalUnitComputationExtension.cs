...
namespace MatthL.PhysicalUnits.Models
{
    public static class PhysicalUnitComputationExtension
    {
        ...
        {
            var function = GetToSiFunction(unit);
        ...
        {
            var toSiFunctions = GetToSiFunction(FromUnit);
            var fromSIFunctions = GetFromSIFunction(toUnit);
            return (double inputValue) =>
            {
                double result = 1.0;
        ...
        {
            var function = GetFromSIFunction(unit);
            return function(value);
        }
        /// <summary>
        /// Convertit un tableau de valeurs vers l'unité SI équivalente
        ...
        {
            var function = GetFromToSiFunction(unit,ToUnit);
            return function(value);
        }
        /// <summary>
        /// Convertit un tableau de valeurs vers l'unité SI équivalente
