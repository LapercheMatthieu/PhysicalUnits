...
namespace  PhysicalUnitManagement.Services
{
    ...
    public static class PhysicalUnitStorage
    {
            ...
            )
        {
            ...
            {
                query = query.Where(u =>
                      (ShowMetrics && u.UnitSystem == StandardUnitSystem.Metrique) ||
                      (ShowImperial && u.UnitSystem == StandardUnitSystem.Imperial) ||
                      (ShowOther && u.UnitSystem == StandardUnitSystem.Autre) ||
                      (ShowUS && u.UnitSystem == StandardUnitSystem.US) ||
                      (ShowAstronomic && u.UnitSystem == StandardUnitSystem.Astronomique) ||
                      (u.UnitSystem == StandardUnitSystem.SI) || 
                      (u.UnitSystem == StandardUnitSystem.Mixte)
                  );
            }
        ...
        public static IEnumerable<PhysicalUnitDomain> GetDomains()
        {
            return Enum.GetValues<PhysicalUnitDomain>()
                .Where(d => d != PhysicalUnitDomain.NonDefini);
        }
